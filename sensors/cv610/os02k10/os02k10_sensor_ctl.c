...
{
	/* Only the default (normal, angle=0) orientation is confirmed against
	 * real hardware — see the OS02K10_REG_FORMAT1/2 comment in the header.
	 * Any non-normal request reproduces the recovered init value rather
	 * than guessing at the alternate 0x0c encoding. */
	if (sns_mirror_flip == ISP_SNS_NORMAL) {
		...
		isp_err_trace("OS02K10: mirror/flip mode %d not confirmed against hardware, ignoring\n",
					  sns_mirror_flip);
	}
}
